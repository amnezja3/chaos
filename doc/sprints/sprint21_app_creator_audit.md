...
jawnego syncu do SQLite. To jest zgodne z `doc/architecture/resource_architecture.md`.
...
| --- | --- | --- | --- |
| 21.5 Gameplay Contract | Runtime checklist, opis wymaganych/opisowych pol aplikacji, status `support_only`/`data_producing` w dokumentach. | `doc/gameplay/app_contract.md`, `doc/gameplay/gameplay_terms.md`, `doc/gameplay/file_model.md` | Audit katalogu, brak zmian mechaniki. |
| 22 Storage | `file_size`, `disk_usage`, `storage_capacity`, usage bar. | `run.py`, `static/js/terminal.js`, `static/css/style.css`, `doc/gameplay/file_model.md` | Install app, generate file, sale flow, no storage regression. |
| 23 Quality | `quality_score`, `reliability`, `creator_power` w kontrakcie i wycenie. | `run.py`, `static/js/terminal.js`, `doc/gameplay/app_contract.md`, `doc/gameplay/data_economy.md` | Two apps same action produce different completeness/price. |
