...
Kontynuacja planu z `doc/history/game_play_260626.md`.

`doc/history/game_play_260626.md` zamyka aktywny plan na Sprincie 103. Ten plik zaczyna
...
Artefakt sprintu: `doc/sprints/sprint_124_ghostnetwork_abilities.md`.
...
Artefakt sprintu: `doc/sprints/sprint_125_ghostnetwork_rewards.md`.
...
Artefakt sprintu: `doc/sprints/sprint_126_ghostnetwork_conflicts.md`.
...
* artefakt `doc/sprints/sprint_127_ghostnetwork_closure.md`.
...
Spójność sprawdzono względem `doc/overview/clans_machines.md`,
`doc/systems/ghostnetwork/ghostnetwork_architecture.md` oraz sprintów 110-126.
...
* aktualizacją `doc/history/project_journal.md`,
...
```text
doc/runbooks/profile_store_migration_manual.md
```
...
* `doc/overview/clans_machines.md`,
* `doc/systems/ghostnetwork/ghostnetwork_architecture.md`,
* `doc/audits/ghost_control_suite_contract_audit.md`,
...
* `doc/history/game_play_180726.md` — Decision, rzeczywisty kontrakt i odchylenia;
* `doc/history/project_journal.md` — zmiany, testy, migracje, flagi i produkcyjne logi;
* `doc/overview/clans_machines.md` — zmiany immunitetu, crew lub własności;
* `doc/systems/ghostnetwork/ghostnetwork_architecture.md` — nowe eventy, audience i hooki;
...
zapisano w `doc/audits/operation_feedback_spike_01_results.md`. Dwie syntetyczne
...
`doc/audits/ofs_presentation_lift_challenger.md`.
...
* aktualizacja `doc/systems/cyberner/cyberner.md`, `doc/audits/cyberner_channels_audit.md` oraz
  `doc/history/project_journal.md` zgodnie z faktycznie wdrożonym stanem.
...
* Sekwencję wdrożenia, obserwowalność i ograniczenia rollbacku opisuje
  `doc/runbooks/cyberner_cutover_runbook.md`. Sprint nie przełącza flag produkcyjnych.
...
`doc/overview/clans_machines.md`; do czasu wdrożenia obecna reguła produkcyjna pozostaje
bez zmian.
...
Zaktualizować `doc/overview/clans_machines.md` i `doc/history/project_journal.md`, zapisując
...
Kanoniczną specyfikację wdrożeniową opisuje `doc/systems/audio-feedback/system_audio.md`, a wyniki
audytu i szersze uzasadnienie architektury pozostają w
`doc/systems/audio-feedback/game_sound_effects_system.md`.
...
`doc/plans/Refaktor_UX_creatorów_CHAOS.md`. Poniższy zapis jest jego adaptacją do
...
- uzupełnione `doc/gameplay/app_contract.md` o mapę etykieta ↔ klucz runtime oraz
  kontrakt ikony;
- `doc/plans/Refaktor_UX_creatorów_CHAOS.md` oznaczony jako artefakt kierunkowy, a ten
  plan jako specyfikacja wykonawcza;
- wpis w `doc/history/project_journal.md` z zakresem, testami i znanymi ograniczeniami;
...
- `doc/gameplay/app_contract.md` opisuje grupy UX oraz ich niezmienne mapowanie na pola
  kontraktu;
- w `doc/history/project_journal.md` zapisano listę zmigrowanych ekranów i wynik macierzy
  regresji;
...
- zaktualizowane `doc/gameplay/app_contract.md`, ten plan oraz
  `doc/plans/Refaktor_UX_creatorów_CHAOS.md`, bez sprzecznych statusów i nazw;
- `doc/history/project_journal.md` zawiera wynik walidacji, znane ograniczenia i informację
...
1. status i wynik sprintu w `doc/history/game_play_180726.md`;
2. kontrakt w `doc/gameplay/app_contract.md`, jeżeli zmieniła się prezentacja lub
   walidacja danych;
3. `doc/history/project_journal.md` z wykonanym zakresem i wynikiem testów;
...
* `doc/systems/ghostnetwork/ghostnetwork_architecture.md` — ownership, przepływ eventów,
  exactly-once/outbox, granice adapterów i source of truth;
* `doc/runbooks/ghostnetwork_endgame_runbook.md` — closure, transmission, retry,
...
* `doc/history/project_journal.md` — zmiany, migracje, uruchomione testy z wynikami,
...
* `doc/history/game_play_180726.md` — status etapów, wynik i nierozwiązane findings,
* `doc/systems/ghostnetwork/ghostnetwork_architecture.md` — tylko jeśli zmienił się kontrakt lub
  przepływ odpowiedzialności,
* `doc/runbooks/ghostnetwork_endgame_runbook.md` — rzeczywiste komendy pre-flight,
  diagnostyki, cleanupu, rollbacku i recovery,
* `doc/history/project_journal.md` — data, zakres, testy, wynik i decyzja GO/NO-GO.
...
9. Po każdym sprincie uruchom testy celowane, właściwą regresję mapy/GN,
   ...
   `git diff --check`.
10. Zaktualizuj tę specyfikację i `doc/history/project_journal.md`. Jeżeli zmienia się
...
- `doc/audits/profile_integrity_writer_inventory.md` — mapa writerów i source of truth;
- `doc/runbooks/profile_integrity_recovery_runbook.md` — wyłącznie serwerowy evidence
...
**Wiążący artefakt:**
`doc/sprints/sprint_130_10_2_marked_target_hot_path.md`.
...
**Bramka heavy-profile:** `doc/architecture/profile_hot_path_contract_130_11_plus.md`.
...
# Sprint 130.12 — GhostNetwork Suite Readiness Cutover

**Status planu:** `IN PROGRESS — STARTED 2026-08-24`.
...
## Dlaczego ten sprint istnieje

Pierwszy audit Sprintu 131 potwierdził, że fundament GhostNetwork Suite już
istnieje: sześć relacji widoczności, frozen conflict context, viewer projection,
`view=suite`, delta projection oraz helper Territory Control. Jednocześnie
wykrył blockery, których nie wolno przenieść do implementacji 132:

1. Territory Control pobiera pełny profil przez `load_profile_readonly()`.
2. `/api/blacknet/cta/teleport` pobiera i zapisuje pełny profil przez
   `UserProfileManager` oraz akceptuje klientowe współrzędne.
3. Alias właściciela nie ma bounded batch projection; pojedyncze odczyty
   tworzyłyby N+1, a `list_profile_identities()` skanuje wszystkich graczy.
4. `GhostNetworkDeltaClient` jest związany z plikiem mapy i wymaga obecności
   mapowego okna do obsługi eventów.
5. Przyszły audience fan-out nie ma lekkiego, indeksowanego resolvera odbiorców.

Sprint 130.12 usuwa wyłącznie te przeszkody infrastrukturalne. Nie buduje
GhostNetwork Suite, nie dodaje jej produktu, kart, filtrów ani GUI. Po jego
zakończeniu Sprint 131 zostaje wykonany ponownie jako niezależna bramka
kontraktowa. Dopiero zielony re-audit otwiera Sprint 132.

## Cel

Przygotować lekkie, canonical i session-safe granice wspólne dla Suite,
Territory Control, mapy, teleportu, delt i późniejszego audience fan-outu tak,
aby zwykłe requesty 132–138 miały:
...
sync_session_profile = 0
```

Wynik nie może zależeć od rozmiaru `profile_json`. Mały profil i syntetyczny
profil co najmniej 35 MB muszą wykonywać ten sam ograniczony zestaw zapytań.

## Zakres i granice odpowiedzialności

### Canonical stores

- instalacja aplikacji: `PlayerInventoryStore`;
- pozycja motocykla: `PlayerPositionStore`;
- aimed target: `PlayerTargetRuntimeStore`;
- targety i obszary: territory/ownership/target stores;
- części, lifecycle i visibility: `GhostNetworkRepository`,
  `GhostModuleStateService`, `GhostVisibilityService`;
- sesja: durable `session_generation` z input i precommit guardem;
- pełny profil: wyłącznie guarded write/audit/recovery, nigdy zwykły snapshot,
  focus, teleport, delta ani fan-out.

### Dozwolone projekcje

- `UserStore.get_profile_identity(username)` dla pojedynczego viewera;
- bounded identity lookup dla jawnego zbioru owner IDs;
- indeksowana identity/recipient projection dla odbiorców clan/owner/player;
- viewer-projected GN `parts[]` i `part_projection`;
- lekkie snapshoty inventory, pozycji, target runtime i Territory Control.

### Zakazane rozwiązania

- fallback do pełnego profilu „tymczasowo”;
- `list_profiles()` lub `list_profile_identities()` w zwykłym runtime;
- wywołanie `get_profile()` per owner/recipient;
- drugi store GhostNetwork, drugi visibility resolver albo drugi snapshot;
- alternatywny endpoint teleportu;
- drugi poller albo drugi klient delt;
- zapis współrzędnych ukrytej części do DOM, cache, logu lub payloadu klienta;
- kopiowanie stanu GN, terytorium albo inventory do sesji jako source of truth;
- synchroniczny rebuild terytorium albo globalny reconcile w request path.

## Etap 0 — freeze, callsite inventory i baseline

Przed zmianą:

1. zapisać aktualny `git status`, HEAD i diff bez cofania zmian użytkownika;
2. zinwentaryzować wszystkie callsites:
   - `build_territory_control_snapshot` i endpointy detail/mutation,
   - `territory_control_load_profile`,
   - `/api/blacknet/cta/teleport`,
   - `GhostNetworkDeltaClient` oraz terminalowy dispatcher,
   - `get_profile_identity`, `list_profile_identities`, `list_profiles`,
   - audience resolver i `GhostNarrativePublisher`;
3. zapisać bieżące query count i `[HOT_PATH]` dla Territory Control oraz
   teleportu na małym i ciężkim profilu;
4. potwierdzić, że testowe odczyty nie mutują bazy;
5. przypisać każdy callsite do `canonical_projection`, `runtime_blocker` albo
   `allowed_offline_heavy`.

Audit ma objąć nie tylko GET-y. Mutacje security, preset, abandon i teleport
muszą używać tego samego lekkiego bootstrapu oraz session precommit guardu.

Bramka:

`SPRINT 130.12 BASELINE CAPTURED`.

## Etap 1 — bounded identity i recipient projection

### Model

Powstaje lekka, indeksowana projekcja tożsamości użytkownika, nie drugi source
of truth. Minimalny kontrakt:
...
Canonical identity pozostaje chronionym profilem. Projekcja:

- jest aktualizowana atomowo przy guarded identity write/rejestracji;
- nie może wyprzedzić revision profilu;
- przy mismatch revision/checksum failuje zamknięta albo zwraca bezpieczny
  fallback bez aliasu;
- nie zawiera inventory, walletu, targetów, pozycji, operations ani GN state;
- ma indeksy co najmniej po `username` i `clan_code`;
- jej migracja jest additive, idempotentna i nie wykonuje template sync;
- bootstrap istniejących rekordów jest jawnym narzędziem migracyjnym, nie
  kosztem pierwszego requestu.

### API store

Wymagane lekkie operacje:

```text
get_identity(username)
get_identities(usernames, max_items=<bounded>)
list_recipient_ids(scope, clan_code=None, owner_ids=None, limit=<bounded>)
```

`get_identities` deduplikuje wejście, posiada jawny limit i wykonuje bounded
batch query, nie N+1. Resolver odbiorców zwraca wyłącznie identyfikatory i
projekcje dozwolone dla audience. Brak indeksu lub przekroczenie limitu kończy
się kontrolowanym błędem/backpressure, nie skanem profili.

### Compatibility

`get_profile_identity()` może pozostać wejściem dla viewera, ale owner aliases i
fan-out korzystają z nowej projekcji. Nie wolno materializować aliasów przez
`list_profile_identities()`.

## Etap 2 — Territory Control zero-profile cutover

`build_territory_control_snapshot(username, ...)` przestaje przyjmować lub
ładować pełny profil. Otrzymuje wyłącznie jawne lekkie zależności:

```text
viewer_identity
inventory/install entitlement
player_position
aimed_target
captured targets
player areas
...
bounded owner aliases
```

### Instalacja

Sprawdzenie `territoryControl` korzysta z canonical inventory. Preferowany jest
punktowy `PlayerInventoryStore.has_app(username, app_id)` albo równoważny
bounded lookup. Zwykły GET nie seeduje inventory z profilu.

### Pozycja i aimed target

- pozycja pochodzi z `PlayerPositionStore.get_position(username)`;
- aimed target pochodzi z `PlayerTargetRuntimeStore.get(username)`;
- brak pozycji lub targetu daje jawny stan `unavailable`, bez compatibility
  overlay pełnego profilu;
- odległości są pochodną snapshotu, nie stanem trwałym.

### GhostNetwork component projection

Territory Control używa istniejącego:
...
Każdy element `ghost_parts[]` jest wynikiem `project_part_for_viewer`. Nie
spłaszczać pierwszej części i nie przekazywać surowego wiersza repository.
Ukryta część nie może ujawnić `part_id`, kodu, ability, właściwego assetu,
`target_id` ani dokładnych współrzędnych.

### Endpointy

Cutover obejmuje:

- GET listy i detail klastra;
- security toggle;
- security preset;
- abandon;
- snapshot zwracany po mutacji.

Każdy endpoint respektuje session generation na wejściu, a mutacja ponownie
sprawdza lineage bezpośrednio przed commit. Brak profilu w cache sesji nie może
zmienić wyniku.

## Etap 3 — canonical teleport i opaque GhostNetwork targets

Nie powstaje nowy endpoint. Rozszerzony zostaje istniejący:

```text
POST /api/blacknet/cta/teleport
```

### Wspólna granica ruchu

Teleport wszystkich wspieranych źródeł zapisuje pozycję przez canonical
`PlayerPositionStore`/movement service. Pełny `load_profile_readonly`,
`UserProfileManager` i compatibility full-profile write są usunięte z requestu.

Zapis wymaga:

- aktualnej `session_generation` na wejściu;
- transakcyjnego precommit guardu;
- bounded writer-locka;
- idempotency/receipt dla retry tego samego teleportu;
- jednej delty `map.player_moved` dla faktycznej zmiany;
- braku synchronicznego territory rebuild/reconcile;
- intrusion check po canonical pozycji bez ładowania profilu.

Compatibility mirror pozycji może zostać odświeżony dopiero przy późniejszym
legalnym guarded profile write. Teleport nie zapisuje całego profilu.

### GhostNetwork Suite source
...
1. odrzuca payload zawierający `lat`, `lng`, `lon`, `latitude` lub `longitude`;
2. pobiera viewer identity z lekkiej projekcji;
3. rozwiązuje opaque ID przez canonical GN/territory stores;
4. ponownie wykonuje `project_part_for_viewer` w bieżącym stanie;
5. dla `location_visibility=exact` wybiera dokładną kotwicę;
6. dla `territory_only` wybiera centroid lub deterministyczny bezpieczny punkt
   aktualnego terytorium;
7. odrzuca cel, który zniknął, zmienił widoczność lub nie należy do viewer
   projection;
8. nie zwraca dokładnej kotwicy ukrytej części w odpowiedzi, logu ani delcie.

Ghost Anchor używa bieżącej canonical historycznej kotwicy tylko wtedy, gdy
visibility contract pozwala ją znać.

### Pozostałe źródła

BlackNet, terminal, Victim Picker i Territory Control zachowują istniejące
zachowanie gameplayowe, lecz również przechodzą przez canonical position write.
Klientowe współrzędne mogą pozostać tylko dla jawnie allowlistowanego,
niesekretnego legacy source i muszą przejść walidację zakresu. Każdy source ma
osobny test kontraktu.

## Etap 4 — wspólny klient delt bez zależności od mapy

Transport, dedupe, version-gap i recovery zostają wydzielone z:

```text
static/js/map/ghostnetwork.js
```

do lekkiego modułu bez Leaflet, przykładowo:

```text
static/js/ghostnetwork_delta_client.js
```

Dokładna nazwa wynika z konwencji bundlingu repo, ale obowiązuje jeden klient i
jedna instancja per desktop/session generation.

Podział odpowiedzialności:

- shared client: odbiór `/api/state/changes`, dedupe, wersje, gap detection,
  recovery snapshot, subscriber lifecycle;
- mapa: wyłącznie adapter renderujący markery i connections;
- przyszłe Suite: subscriber modelu części, bez importu Leaflet;
- SFX: tylko live delta po przejściu istniejącej bramki;
- snapshot/recovery: zawsze ciche.

Mapa i terminal zostają przełączone na nowy moduł w tym samym cutoverze. Nie
pozostawiać równoległego klienta w mapie i nie dodawać pollera.

Teardown przy logout/account switch czyści subskrypcje, cursor, dedupe, recovery
promise i viewer-projected cache. Stara generation nie może zastosować delty.

## Etap 5 — audience recipient readiness

Przyszły Sprint 136 nie może rozpocząć fan-outu od skanowania profili. Sprint
130.12 dostarcza wyłącznie neutralną infrastrukturę resolvera odbiorców:

- public: jeden logiczny audience task, bez listy pełnych profili;
- clan: indeksowany odczyt IDs dla konkretnego `clan_code`;
- owner: jawny bounded zbiór owner IDs;
- player: jeden jawny player ID;
- internal/system: brak publikacji do klienta.

Resolver nie buduje jeszcze faktów narracyjnych i nie zmienia
`GhostNarrativePublisher`. Udostępnia bezpieczny kontrakt, limit, pagination lub
lease-friendly batch oraz metryki liczby odbiorców bez danych profilu.

Migration/backfill projekcji identity ma osobne `status`, `audit`, `dry-run`,
`apply` i `verify`. `apply` wymaga jawnego polecenia operatorskiego; start weba i
workera nie wykonuje backfillu automatycznie.

## Etap 6 — telemetryka i fail-closed observability

Rozszerzyć telemetrykę `[HOT_PATH]` dla:

```text
GET /api/ghost-control/territory
GET /api/ghost-control/territory/<cluster_id>
...
sqlite_writer_wait_ms
bounded_identity_count
recipient_batch_count
request_ms
```

Nie logować username, aliasów, współrzędnych, opaque ID, session generation ani
pełnych payloadów. Do korelacji używać jednokierunkowych bounded hashy i
request ID.

Nieoczekiwany heavy path w endpointach objętych sprintem ma być widoczny w
teście i telemetryce oraz kończyć bramkę NO-GO. Telemetryka nie może sama
wydłużać writer-locka ani wykonywać dodatkowego profilu read.

## Migracje

Dozwolona jest jedna additive migracja lekkiej identity/recipient projection i
indeksów. Musi być:

- idempotentna;
- zgodna ze starą bazą bez tabeli/kolumn;
- fail-closed dla invalid/recovery-required profilu;
- bez zmiany `profile_revision` i bez promocji LKG;
- bez seedowania walletu, inventory, pozycji, terytoriów lub GN;
- bez długiego `BEGIN IMMEDIATE` — dane przygotować przed lockiem i zapisywać
  bounded batchami;
- bez automatycznego uruchomienia podczas zwykłego requestu;
- objęta backup/rollback runbookiem przed produkcyjnym apply.

Jeżeli aktualne `users` columns/index mogą zapewnić te same bounded lookupy bez
nowej tabeli i bez parsowania pełnego JSON, audit Etapu 0 może wybrać mniejszą
migrację. Decyzja i pomiar muszą trafić do artefaktu sprintu.

## Testy automatyczne

### Identity i recipient projection

- bounded batch zwraca tylko żądane identities;
- przekroczenie limitu failuje bez zapytania all-user;
- brak N+1 niezależnie od liczby ownerów;
- clan lookup korzysta z indeksu;
- invalid/recovery-required profile nie zasila projekcji;
- projection revision/checksum drift jest wykryty;
- backfill dry-run nie zmienia pliku bazy;
- retry apply nie dubluje rekordów ani nie zwiększa profile revision.

### Territory Control

- list/detail/mutation snapshot: zero `load_profile_readonly`, `get_profile`,
  `get_profile_with_revision`, `sync_session_profile` i `UserProfileManager`;
- entitlement pochodzi z inventory, pozycja z position store, aimed target z
  runtime store;
- hidden GN part nie ujawnia identity/ability/asset/target/coordinates;
- owner, clan member, foreign viewer i conflict zachowują macierz visibility;
- `ghost_parts[]` zawiera wyłącznie viewer projection;
- brak GN nie zmienia dotychczasowego snapshotu terytorium;
- mutacja starej karty zostaje odrzucona przed commit.

### Teleport

- `source=ghostnetwork_suite` odrzuca każde pole współrzędnych klienta;
- exact viewer trafia do exact anchor;
- territory-only viewer trafia do bezpiecznego punktu bez wycieku anchor;
- visibility drift pomiędzy resolve i commit zatrzymuje teleport;
- nieznany/stary opaque ID daje kontrolowane 404/409 bez mutacji;
- retry z tym samym kluczem nie tworzy drugiego move/delta;
- stara session generation przegrywa input i precommit;
- wszystkie źródła wykonują canonical position write bez pełnego profilu;
- mapa, intrusion i odległości odświeżają się bez synchronicznego rebuilda.

### Shared delta client

- mapa i subscriber testowy używają jednej instancji;
- jedna delta aktualizuje część raz;
- duplicate i out-of-order nie cofają wersji;
- version gap pobiera właściwy snapshot i nie odtwarza SFX;
- zmiana visibility usuwa stare pola z cache przed powiadomieniem subscriberów;
- logout/account switch abortuje recovery i czyści state;
- import shared client nie importuje Leaflet ani nie otwiera mapy.

### Heavy-profile gate

Uruchomić każdy zwykły endpoint na:

1. małym profilu;
2. syntetycznym profilu co najmniej 35 MB.

Dla obu wariantów wymagane są identyczne:

- query count w granicach ustalonego budżetu;
- liczba odczytanych identity/recipient rows;
- liczba zapisów;
- payload shape i wynik gameplayu.
...
Testy statyczne mają blokować ponowne dodanie zakazanych helperów do objętych
funkcji i endpointów.

## Regresja obowiązkowa

Minimum:

- profile write guard, CAS, checksum i LKG;
- session generation store/isolation/precommit oraz frontend teardown;
- wallet i inventory canonical boundary;
- hot path recovery i marked target;
- Territory Control, Victim Picker i Operation Control;
- target persistence, position/movement i intrusion;
- GhostNetwork visibility, module state, snapshot, delta publisher/audience,
  map renderer, SFX silence i read-path safety;
- BlackNet CTA oraz istniejące teleporty;
- territory/conflict visibility i worker fairness.

Po zmianach:

- `py_compile` dla zmienionych plików Python;
- `node --check` dla zmienionych plików JS;
- wszystkie celowane testy Python/Node;
- pełna adekwatna regresja;
- `git diff --check`.

## Manual i pomiar serwerowy

Po zielonych automatach, osobnym deployu i restarcie zleconym przez użytkownika:

1. Territory Control na małym oraz ciężkim koncie — list, detail, security,
   preset i abandon testowego obiektu;
2. BlackNet/terminal/Victim Picker/Territory Control teleport — pozycja, delta,
   odległości i intrusion;
3. dwie karty i dwa konta — stara karta nie przesuwa nowego gracza;
4. mapa zamknięta — shared delta client nie ładuje Leaflet;
5. mapa otwarta — markery i GN delty działają bez duplikatów;
6. version gap/recovery — brak historycznego SFX;
7. heavy account — brak regresji latency i wszystkie liczniki profilu równe 0;
8. PM2/web/worker — brak restartów, tracebacków i `database is locked` storm.

Manual nie używa prawdziwej ukrytej części do ujawniania sekretów. Scenariusze
exact/territory-only realizować fixture'em lub zatwierdzonym testowym stanem.

## Bramka operatorska

Sprint nie daje automatycznej zgody na:

- deploy;
- restart PM2;
- migrację lub backfill produkcyjnej bazy;
- zmianę flag GhostNetwork;
- mutację cyklu, części, terytoriów albo profilu;
- commit lub push.

Przed produkcyjną migracją identity projection:

```text
status → audit → dry-run → backup → apply → verify
```

Każdy drift schema, profile revision/checksum, session lineage albo liczby
projekcji kończy operację fail-closed i wymaga nowego planu.

## Artefakty Sprintu 130.12

Sprint tworzy lub aktualizuje:

- techniczny artefakt `doc/sprints/sprint_130_12_suite_readiness_cutover.md`;
- inventory callsites i wyniki heavy-profile w tym artefakcie;
- runbook migracji identity projection, jeżeli migracja będzie potrzebna;
- `doc/history/project_journal.md`;
- tę roadmapę;
- testy kontraktowe i narzędzie `status/audit/dry-run/verify` dla nowej projekcji.

Nie wpisywać wyników testów ani statusu COMPLETE przed ich rzeczywistym
wykonaniem.

## Definition of Done

Sprint 130.12 jest zakończony, gdy:

1. Territory Control list/detail/mutations nie czytają ani nie zapisują pełnego
   profilu.
2. Instalacja, pozycja i aimed target pochodzą z canonical stores.
3. Owner aliases są rozwiązywane bounded batch bez N+1 i all-user scan.
4. Istnieje indeksowany recipient resolver dla player/owner/clan/public bez
   pełnych profili.
5. Teleport wszystkich wspieranych źródeł zapisuje canonical position bez
   pełnego profile write.
6. GhostNetwork teleport odrzuca klientowe coordinates i rozwiązuje opaque cel
   po aktualnej visibility projection.
7. Ukryta kotwica nie trafia do payloadu, DOM, logu, cache ani delty.
8. Mapa i przyszłe Suite mogą użyć jednego shared delta client bez Leaflet.
9. Snapshot/recovery pozostają ciche, a live SFX nie dubluje się.
10. Session generation input/precommit chroni wszystkie objęte mutacje.
11. Mały i profil 35 MB mają identyczny bounded query count oraz wszystkie
    heavy-profile counters równe zero.
12. Nie powstał nowy source of truth, poller, endpoint teleportu ani snapshot.
13. Celowane testy, regresja, składnia i `git diff --check` są zielone.
14. Manual i serwerowy before/after nie wykazują latency, lock contention,
    wycieku visibility ani cross-account state.
15. Audit Sprintu 131 został ponowiony przeciw wdrożonemu kodowi i nie zawiera
    otwartego blockera dla 132.

Bramka lokalna:

`SPRINT 130.12 IMPLEMENTED — READY FOR SERVER VALIDATION`.

Bramka po manualu i verify:

`SPRINT 130.12 — COMPLETE`.

Następnie ponowić audit Sprintu 131. Oczekiwany werdykt:

`SPRINT 131 — COMPLETE`

`READY FOR SPRINT 132`.

Jeżeli którykolwiek heavy-profile, privacy, session, migration lub delta
blocker pozostaje otwarty:

`NO-GO — SPRINT 130.12 READINESS BLOCKERS REMAIN`.

---

# Bramka przed Sprintem 131 i 132

Nie używamy nazw `130.9.6` i `130.9.7`, ponieważ występują już jako historyczne
etapy Runtime Enablement. Aktualna kolejność jest jednoznaczna:

```text
130.10 — integralność profilu i izolacja sesji — COMPLETE
→ 130.11 — kontrolowana odbudowa Trollu2 — COMPLETE
→ pierwszy audit 131 — blockery zidentyfikowane
→ 130.12 — GhostNetwork Suite Readiness Cutover
→ ponowny audit 131 — wymagane READY FOR SPRINT 132
→ 132 — lekki snapshot GhostNetwork Suite
```

Do czasu COMPLETE 130.12 i zielonego re-auditu 131:

`SPRINT 132 — BLOCKED`.

---

Kontynuacja w `doc/history/game_play_240826.md`.
