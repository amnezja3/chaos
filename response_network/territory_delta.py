...
def _conflict_payload(conflict, reason=""):
    ...
    area_ids = [
        item
        for item in (conflict.get("area_ids") or conflict.get("primary_area_ids") or [])
        if item not in (None, "")
    ]
    return {
        "conflict_id": conflict.get("id"),
        "conflict_key": _clean_text(conflict.get("conflict_key"), "unknown"),
        "participants": participants,
        "area_ids": area_ids,
        "status": _clean_text(conflict.get("status"), "active"),
        "updated_at": _clean_text(conflict.get("updated_at") or conflict.get("created_at")),
        "version": _clean_text(conflict.get("updated_at") or conflict.get("id") or conflict.get("conflict_key"), "0"),
        "reason": _clean_text(reason),
    }
...
class TerritoryDeltaPublisher:
    ...
    def record_conflict_changed(self, conflict, reason=""):
        ...
        conflict_key = payload.get("conflict_key") or "unknown"
        version = payload.get("version") or "0"
        participants = payload.get("participants") or []
        events = []
        for username in participants:
            key = "territory:conflict:{username}:{conflict_key}:{version}:{reason}".format(
                username=username,
            ...
            events.append(self.delta_bus.record_change(
                ...
                payload,
                entity_id=f"territory_conflict:{conflict_key}",
                dedupe_key=key,
            ))
        return events
