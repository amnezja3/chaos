...
(async()=>{
    ...
    assert(!audio.paused);
    audio.seeking=true;audio.currentTime=1;radio.syncShow(at(425));
    assert.strictEqual(audio.currentTime,1,'audio seek in progress is not restarted');
    audio.seeking=false;radio.syncShow(at(425));
    advance(1750);
