...
assert.match(catalogSource, /gp-search-card gp-search-card--\$\{weight\}/);
assert.match(catalogSource, /class="gp-search-card__content"/);
assert.match(catalogSource, /class="gp-search-card__asset"/);
assert.match(catalogSource, /class="gp-search-card__action"/);
assert.match(catalogSource, /<img src="\$\{escapeHTML\(safeAsset\)\}"/);
assert.match(catalogSource, /gp-search-card__asset-icon/);
...
assert.match(styles, /\.gp-search-card__asset img[\s\S]*object-fit: contain/);
...
assert.match(styles, /@container \(max-width: 767px\)[\s\S]*\.gp-search-grid \{ display: flex; flex-direction: column/);
assert.match(styles, /\.browser-window\.browser-narrow\.is-browser-googleplex \.googolplex-grid[\s\S]*overflow-y: visible !important/);
assert.doesNotMatch(styles.slice(styles.indexOf("/* Search belongs")), /background-image\s*:/);
