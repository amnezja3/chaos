...
class NarrativeCutoverTest(unittest.TestCase):
    ...
    def test_cutover_fails_closed_on_invalid_runtime_safety_contract(self):
        repository = CutoverRepositoryFixture()
        with patch(
            "ghostnetwork.narrative_cutover.verify_prompt_registry",
            return_value={"ok": True, "errors": []},
        ), patch(
            "ghostnetwork.narrative_cutover.verify_ollama_runtime_policy",
            return_value={"ok": False, "errors": ["retry_contract_invalid"]},
        ):
            report = build_narrative_cutover_report(
                repository, config=self.enabled, now=self.now
            )
        ...
        self.assertFalse(report["ok"])
        self.assertIn("ollama_runtime_safety_invalid", report["errors"])
