...
class OllamaEcosystemShutdownContractTest(unittest.TestCase):
    def test_pm2_kill_timeout_exceeds_bounded_model_read_timeout(self):
        source = (Path(__file__).resolve().parents[1] / "ecosystem.ollama-worker.config.js").read_text(encoding="utf-8")
    ...
    def test_worker_remains_single_instance_with_ecosystem_owned_enablement(self):
        source = (Path(__file__).resolve().parents[1] / "ecosystem.ollama-worker.config.js").read_text(encoding="utf-8")
